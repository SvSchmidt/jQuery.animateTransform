...
(function($) {
	$.fn.animateTransform = function(settings) {
		var $elem = this,_settings = $.extend({},$.fn.animateTransform.defaultSettings,settings || {}),
			x=0,y=0,z=0,skewX=0,skewY=0,scale=0,translateLine,rotateLine,skewLine,scaleLine,transformLine,propNum = 0,
		...
		$($elem).animate({
			...
			(propNum == 4 && (rotateLine = "rotate(" + fx.now + "deg)"));		 
			...
			transformLine = (translateLine || "") +  (rotateLine || "") + (skewLine || "") + (scaleLine || "scale(" + objTransform.scale[0] + ")");
			$($elem).css({
				"WebkitTransform":transformLine,
				"MozTransform":transformLine,
				"MSTransform":transformLine,
				"OTransform":transformLine,
