...
(function($) {
	$.fx.step["transform"] = function(fx) {
		...
		rotateLine = "rotate(" + (+(fx.start.rotate + ((fx.end.rotate - fx.start.rotate) * divisor)) || fx.start.rotate) + "deg)";
		scaleLine = "scale(" + (+(fx.start.scale + ((fx.end.scale - fx.start.scale) * divisor)) || fx.start.scale) + ")";
		...
		$elem.css(vendorPrefix + "Transform",
			(translateLine || "translate" + (fx.start.z > 0 ? "3d" : "") + (fx.start.x + "px," + fx.start.y + "px" + (fx.start.z > 0 ? fx.start.z + ",px" : "") + ")")) + (scaleLine) + (rotateLine) + (skewLine || "skew(" + fx.start.skewX + "deg," + fx.start.skewY + "deg)"));
